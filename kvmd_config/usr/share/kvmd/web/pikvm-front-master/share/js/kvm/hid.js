...
export function Hid(__getGeometry, __recorder) {
	...
	var __clickPasteAsKeysButton = function() {
		...
		if (text) {
				...
				let http = tools.makeRequest("POST", `/api/hid/print?limit=0&keymap=${keymap}`, function() {
					if (http.readyState === 4) {
						...
						if (http.status === 413) {
							...
							wm.error("HID 解析错误:<br>", http.responseText);
						} else if (http.status === 200) {
			...
			if ($("hid-pak-ask-switch").checked) {
				let confirm_msg = `您将粘贴 ${text.length} 个字符到本地界面，<br>`;
				confirm_msg += "是否确定执行此操作？";
				wm.confirm(confirm_msg).then(function(ok) {
					if (ok) {
						paste_as_keys();
					} else {
	...
	var __clickResetButton = function() {
		wm.confirm("是否确定重启键鼠信号").then(function(ok) {
			if (ok) {
	...
	self.setKVMControl = function ({ mouse, keyBoard }) {
		__mouse.enabledOrDisabledMouse(mouse)
		__keyboard.setEnabled(keyBoard)
	}

	self.setFeatureEnabled = function (enabled) {
		tools.feature.setEnabled($('hid-dropdown'), enabled)
	}

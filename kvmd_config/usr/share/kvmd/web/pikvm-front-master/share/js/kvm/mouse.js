...
export function Mouse(__getGeometry, __recordWsEvent) {
	...
	var __setLocalMouse = function() {
		const change = function (mouse) {
			const streamBox = $("stream-box")

			streamBox.classList.remove("stream-box-mouse-enabled", "stream-box-mouse-disabled")
			
			if (mouse === 'circle') {
				streamBox.classList.add("stream-box-mouse-enabled")
			} else if (mouse === 'hidden') {
				streamBox.classList.add("stream-box-mouse-disabled")
			}
		}

		tools.storage.bindRadio('local-mouse', 'local.mouse.mode', 'circle', change)
	}

	self.enabledOrDisabledMouse = function (enabled) {
		tools.el.setEnabled($('hid-mute-switch'), enabled)
		$('hid-mute-switch').checked = enabled
	}

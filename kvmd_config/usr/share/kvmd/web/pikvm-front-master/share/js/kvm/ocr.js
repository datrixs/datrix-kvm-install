...
import {hasPermission, permissionCodeMap} from "../getUserInfo.js";
...
export function Ocr(__getGeometry) {
	...
	self.setState = function(state) {
		// 权限：OCR
		let enabled = (state && state.ocr.enabled && !tools.browser.is_ios && hasPermission(permissionCodeMap.ocr));
		if (enabled) {
	...
	var __recognizeSelection = function() {
		...
		let http = tools.makeRequest("GET", url, function() {
			if (http.readyState === 4) {
				if (http.status === 200) {
					const success = copy(http.responseText)
					...
					wm.error("OCR 识别错误:<br>", http.responseText);
				}
	...
	self.setOcrFeatureEnabled = function (enabled) {
		tools.feature.setEnabled($("stream-ocr"), enabled)
	}

...
import { onMessage, getWsUrl, inUpgrading } from '../upgrade.js'
...
export function Session() {
  ...
  self.glob_recorder = __recorder;
  /************************************************************************/

  var __setAboutInfoMeta = function (state) {
    if (state !== null) {
      let text = JSON.stringify(state, undefined, 4)
        .replace(/ /g, "&nbsp;")
        .replace(/\n/g, "<br>");
      ...
      if (state.server && state.server.host) {
        ...
        document.title = `KVM: ${state.server.host}`;
      } else {
  ...
  var __setAboutInfoHw = function (state) {
    if (state.health.throttling !== null) {
      ...
      tools.hidden.setVisible(
        $("hw-health-dropdown"),
        undervoltage || freq_capped
      );
      $("hw-health-undervoltage-led").className = undervoltage
        ? flags.undervoltage.now
          ? "led-red"
          : "led-yellow"
        : "hidden";
      $("hw-health-overheating-led").className = freq_capped
        ? flags.freq_capped.now
          ? "led-red"
          : "led-yellow"
        : "hidden";
      tools.hidden.setVisible(
        $("hw-health-message-undervoltage"),
        undervoltage
      );
  ...
  var __setAboutInfoFan = function (state) {
    ...
    $("fan-health-led").className = failed
      ? failed_past
        ? "led-yellow"
        : "led-red"
      : "hidden";
  ...
  var __formatFan = function (state) {
    if (!state.monitored) {
      ...
      let pairs = [
        [
          "Status",
          state.fan.ok ? __colored("green", "Ok") : __colored("red", "Failed"),
        ],
      ...
      if (state.hall.available) {
        pairs.push([
          "RPM",
          __colored(state.fan.ok ? "green" : "red", state.hall.rpm),
        ]);
      }
    ...
    if (throttling !== null) {
      ...
      for (let field of Object.keys(throttling.parsed_flags).sort()) {
        ...
        pairs.push([
          ...
          (flags["now"]
            ? __colored("red", "RIGHT NOW")
            : __colored("green", "No")) +
            "; " +
            (flags["past"]
              ? __colored("red", "In the past")
              : __colored("green", "Never")),
        ]);
      }
      return __formatUl(pairs);
    } else {
  ...
  var __setAboutInfoSystem = function (state) {
    $("about-version").innerHTML = `
			...
			<hr>
			Streamer: <span class="code-comment">${state.streamer.version} (${
      state.streamer.app
    })</span>
  ...
  var __setExtras = function (state) {
    ...
    let close_hook = null;
    let has_webterm =
      state.webterm && (state.webterm.enabled || state.webterm.started);
    if (has_webterm) {
    ...
    __streamer.setJanusEnabled(
      (state.janus && (state.janus.enabled || state.janus.started)) ||
        (state.janus_static &&
          (state.janus_static.enabled || state.janus_static.started))
    );
  };

  var __initWs = function (query) {
    // is_control 后端逻辑依赖
    __ws = new WebSocket(getWsUrl({ ...query, current_page: 'kvm' }));
  ...
  var __startSession = function () {
    ...
    Promise.all([
      ...
      initUserInfo(true, true)
    ]).then(() => {
      const payload = getQueryString('payload')
      
      if (payload) {
        const { equipment_id, operator_id, full_kvm } = JSON.parse(window.atob(payload))

        __enterFull(full_kvm)
        __initWs({ operate_id: operator_id, equipment_id })
      } else {
        __initWs(null)
      }

      // 权限：远程控制 
      const kvmControl = hasPermission(permissionCodeMap.remoteControl)
      __hid.setKVMControl({ mouse: kvmControl, keyBoard: kvmControl })
      __hid.setFeatureEnabled(hasPermission(permissionCodeMap.remoteControl))

      // 权限：OCR 
      __ocr.setOcrFeatureEnabled(hasPermission(permissionCodeMap.ocr))
    }).catch(err => {
      const status = err && err.status
      const msg = err && err.message

      if (status === 401 || status === 403 || msg === '_NEED_LOGIN_') {
        window.onbeforeunload = () => null;
        wm.error("发生意外退出，请重新登录").then(
          function () {
  ...
  var __wsMessageHandler = function (event) {
    // 权限：查看视频流
    if (!hasPermission(permissionCodeMap.viewVideoStream)) return

    let data = JSON.parse(event.data);

    onMessage(data, () => __ws.close())

    switch (data.event_type) {
      case "pong":
        __missed_heartbeats = 0;
        break;
      case "info_meta_state":
        __setAboutInfoMeta(data.event);
        break;
      case "info_hw_state":
        __setAboutInfoHw(data.event);
        break;
      case "info_fan_state":
        __setAboutInfoFan(data.event);
        break;
      case "info_system_state":
        __setAboutInfoSystem(data.event);
        break;
      case "info_extras_state":
        __setExtras(data.event);
        break;
      case "gpio_model_state":
        __gpio.setModel(data.event);
        break;
      case "gpio_state":
        __gpio.setState(data.event);
        break;
      case "hid_keymaps_state":
        __hid.setKeymaps(data.event);
        break;
      case "hid_state":
        __hid.setState(data.event);
        break;
      case "atx_state":
        __atx.setState(data.event);
        break;
      case "msd_state":
        __msd.setState(data.event);
        break;
      case "streamer_state":
        __streamer.setState(data.event);
        break;
      case "streamer_ocr_state":
        __ocr.setState(data.event);
        break;
      case "lock_kvm":
        // data.event: true false
        __hid.setKVMControl({ mouse: data.event, keyBoard: data.event });
        break;
      case "button_permission":
        __setRightTopOpera(data.event);
        break;
      case "control_permission":
        __setControl(data.event);
        break;
    }
  };
  ...
  var __setRightTopOpera = function (data) {
    const { text, macro, stream, control, power, camera } = data

    tools.feature.setEnabled($('feature-ocr'), text)
    tools.feature.setEnabled($('feature-ocr-line'), text)

    tools.feature.setEnabled($('feature-macro'), macro)
    tools.feature.setEnabled($('feature-macro-line'), macro)

    tools.feature.setEnabled($('feature-recoder'), camera)
    tools.feature.setEnabled($('feature-recoder-line'), camera)

    tools.feature.setEnabled($('feature-stream'), stream)
    tools.feature.setEnabled($('feature-stream-line'), stream)

    __hid.setFeatureEnabled(control)
    tools.feature.setEnabled($('hid-dropdown-line'), control)

    tools.feature.setEnabled($('feature-power'), power)
    tools.feature.setEnabled($('feature-power-line'), power)

    __hid.setKVMControl({ mouse: control, keyBoard: control });
  }

  var __enterFull = function (enter) {
    // 参考 wm.js 110
    const el_enter_full_tab_button = $('stream-window').querySelector(".window-header .window-button-enter-full-tab");

    if (enter && el_enter_full_tab_button) {
      el_enter_full_tab_button.click()
    }
  }

  var __setControl = function(data) {
    const control = data.control

    tools.feature.setEnabled($('stream-occupied'), !control)
  }

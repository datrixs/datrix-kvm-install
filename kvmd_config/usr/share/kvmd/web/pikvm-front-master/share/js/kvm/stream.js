...
function _JanusStreamer(__setActive, __setInactive, __setInfo) {
  ...
  var __ensureJanus = function (internal) {
    if (__janus === null && !__stop && (!__ensuring || internal)) {
      ...
      __janus = new _Janus({
        server: `${tools.is_https ? "wss" : "ws"}://${location.host}/janus/ws?auth_token=${tools.auth_token}`,
        ipv6: true,
        ...
        error: function (error) {
          ...
          __finishJanus();
          __ensureChecking(true)
        },
      });
    }
  };

  var __ensureChecking = function (open) {
    document.getElementById("kvm-video").style.display = open ? 'block' : "none";
  };
      ...
      onmessage: function (msg, jsep) {
        ...
        if (msg.result) {
          ...
          __setInfo(
            false,
            false,
            msg.error_code === 503 ? "Waiting for keyframe ..." : msg.error
          );
...
function _MjpegStreamer(__setActive, __setInactive, __setInfo) {
  ...
  var __findId = function () {
    ...
    if (
      __id.length === 0 &&
      stream_client &&
      stream_client.startsWith(__key + "/")
    ) {
      __logInfo("Found acceptable stream_client cookie:", stream_client);
      __id = stream_client.slice(stream_client.indexOf("/") + 1);
    }

    // 内嵌模式下cookie会获取不到 此时通过 WebSocket推送过来的消息进行 key 值匹配进行查找
    if (!__id && __state.stream && __state.stream.clients_stat) {
      const clients_stat = __state.stream.clients_stat

      for (const id in clients_stat) {
        if (clients_stat[id] && clients_stat[id].key === __key) {
          __id = id
          break
        }
      }
  ...
  var __checkStream = function () {
    ...
    if (__id.legnth > 0 && __id in __state.stream.clients_stat) {
      ...
      let path = `/streamer/stream?key=${__key}&auth_token=${tools.auth_token}`;
      ...
      // $("stream-image").src = `http://localhost:3020${path}`;
      setTimeout(function () {
        document.getElementById("kvm-video").style.display = "none";
      }, 200);
    }
  };
...
export function Streamer() {
  ...
  var __init__ = function () {
    ...
    __camera = new Camera(function () { return __streamer })
    ...
    tools.slider.setOnUpDelayed($("stream-quality-slider"), 1000, (value) =>
      __sendParam("quality", value)
    );

    tools.slider.setParams(
      $("stream-h264-bitrate-slider"),
      25,
      20000,
      25,
      5000,
    ...
    tools.slider.setOnUpDelayed(
      $("stream-h264-bitrate-slider"),
      1000,
      (value) => __sendParam("h264_bitrate", value)
    );

    tools.slider.setParams(
      $("stream-desired-fps-slider"),
      0,
      120,
      1,
      0,
      function (value) {
        $("stream-desired-fps-value").innerHTML =
          value === 0 ? "Unlimited" : value;
      }
    );
    tools.slider.setOnUpDelayed($("stream-desired-fps-slider"), 1000, (value) =>
      __sendParam("desired_fps", value)
    );
  ...
  self.getGeometry = function () {
    ...
    let ratio = Math.min(
      res.view_width / res.real_width,
      res.view_height / res.real_height
    );
    return {
  ...
  self.setJanusEnabled = function (enabled) {
    ...
    if ($("stream-video").canPlayType) {
      has_h264 = $("stream-video").canPlayType(
    ...
    let set_enabled = function () {
      ...
      tools.info(
        `Stream: Janus WebRTC state: enabled=${enabled}, webrtc=${has_webrtc}, h264=${has_h264}, imported=${!!_Janus}`
      );
      tools.radio.clickValue(
        "stream-mode-radio",
  ...
  var __applyState = function (state) {
    if (state) {
      tools.feature.setEnabled(
        $("stream-quality"),
        state.features.quality &&
          (state.streamer === null || state.streamer.encoder.quality > 0)
      );
      tools.feature.setEnabled(
        $("stream-h264-bitrate"),
        state.features.h264 && __janus_enabled
      );
      tools.feature.setEnabled(
        $("stream-resolution"),
      ...
      if (state.streamer) {
        ...
        tools.slider.setValue(
          $("stream-quality-slider"),
          state.streamer.encoder.quality
        );

        if (state.features.h264 && __janus_enabled) {
          __setMinMax(
            $("stream-h264-bitrate-slider"),
            state.limits.h264_bitrate
          );
          tools.el.setEnabled($("stream-h264-bitrate-slider"), true);
          tools.slider.setValue(
            $("stream-h264-bitrate-slider"),
            state.streamer.h264.bitrate
          );
        }
        ...
        let resolution_str = __makeStringResolution(
          state.streamer.source.resolution
        );
        ...
        if (state.features.resolution) {
          if (
            $("stream-resolution-selector").resolutions !==
            state.limits.available_resolutions
          ) {
          ...
          document.querySelector(
            `#stream-resolution-selector [value="${resolution_str}"]`
          ).selected = true;
  ...
  var __setInfo = function (is_active, online, text) {
    ...
    __camera.setEnabledOrDisabled(is_active && online)
  };
  ...
  var __clickModeRadio = function () {
    if (_Janus !== null) {
      ...
      // 设置选中态
      $('stream-mode').querySelectorAll('.radio').forEach(el => el.classList.toggle('active', false))
      document.querySelector('input[type="radio"][name="stream-mode-radio"]:checked').parentElement.classList.toggle('active', true)
  ...
  var __clickScreenshotButton = function () {
    ...
    el_a.href = `/api/streamer/snapshot?allow_offline=1&auth_token=${tools.auth_token}`;
    el_a.target = "_blank";
  ...
  var __clickResetButton = function () {
    wm.confirm("是否确定重启视频信号").then(function (ok) {
      if (ok) {
        if (wm.isWindowVisible($("stream-window"))) {
          __streamer.stopStream();
          __streamer.ensureStream(__state);
        ...
        let http = tools.makeRequest(
          ...
          function () {
            if (http.readyState === 4) {
              if (http.status !== 200) {
                wm.error("重启失败:<br>", http.responseText);
              }
            }
          }
        );
  ...
  var __sendParam = function (name, value) {
    let http = tools.makeRequest(
      "POST",
      `/api/streamer/set_params?${name}=${value}`,
      function () {
        if (http.readyState === 4) {
          if (http.status !== 200) {

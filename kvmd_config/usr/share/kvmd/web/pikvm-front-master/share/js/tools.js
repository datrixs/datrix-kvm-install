...
/**
 * https://developer.mozilla.org/zh-CN/docs/Web/API/Window/localStorage#%E5%BC%82%E5%B8%B8
 * 无痕模式下(浏览器设置为阻止第三方Cookie)
 * localStorage 的使用将会导致异常 此时使用内存兜底
 */
const __storage = JSON.parse(window.atob(getQueryString('storage') || 'e30='))

export var tools = new (function () {
  var self = this;

  self.auth_token = ''; // 以iframe内嵌形式下cookie会失效

  self.setToken = function (token) {
    self.storage.set('Authorization', token)
  ...
  self.makeRequest = function (
    ...
    // http.setRequestHeader("X-KVMD-User", "admin");
    // http.setRequestHeader("X-KVMD-Passwd", "admin");
    if (self.auth_token) {
      http.setRequestHeader("Authorization", self.auth_token);
    }
  ...
  self.fetch = function (url, options = {}) {
    return new Promise((resolve, reject) => {
      const { method = 'get', body, contentType = 'application/json', timeout } = options
      const isJson = contentType === 'application/json'
      const http = self.makeRequest(
        method, 
        url, 
        function () {
        ...
        isJson && body ? JSON.stringify(body) : body,
        contentType, 
        timeout
      )
  ...
  self.formatSize = function (size) {
    if (size > 0) {
      ...
      return (
        (size / Math.pow(1024, index)).toFixed(2) * 1 +
        " " +
        ["B", "KiB", "MiB", "GiB", "TiB"][index]
      );
    } else {
      return 0;
    }
  ...
  self.el = new (function () {
    return {
      setOnClick: function (el, callback, prevent_default = true, need_event = false) {
        el.onclick = el.ontouchend = function (event) {
          if (prevent_default) {
            event.preventDefault();
          }
          callback(need_event ? event : void 0);
        };
      },
  ...
  self.input = new (function () {
    return {
      ...
      getFiles: function (el) {
        return el.files.length ? Array.from(el.files) : null;
      },
    };
  })();
  ...
  self.storage = new (function () {
    return {
      ...
      getBool: (key, default_value) =>
        !!parseInt(self.storage.get(key, default_value ? "1" : "0")),
      ...
      bindRadio: function(name, key, default_value, callback = null) {
        let value = self.storage.get(key, default_value);
        self.radio.setValue(name, value)
        if (callback) {
          callback(value);
        }
        self.radio.setOnClick(
          name, 
          function () {
            const val = self.radio.getValue(name)
  ...
  self.auth_token = self.storage.get('Authorization', '')
})();
...
export function getRedirect(path = '/login', query = {}) {
  const search = new URLSearchParams(location.search)

  search.delete('redirect')
  search.delete('storage')
  search.delete('from')
  search.delete('auto')
  ...
  search.append('storage', window.btoa(JSON.stringify(__storage)))
  search.append('from', location.pathname)

  if (path.indexOf('login') === -1) search.delete('redirect')

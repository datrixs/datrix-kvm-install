...
function __WindowManager() {
	...
	var __init__ = function() {
		...
		for (let el_button of $$("menu-button")) {
			const menu = el_button.parentElement.querySelector(".menu")
			if (menu) {
		...
		for (let el_window of $$("window")) {
			...
			if (el_enter_full_tab_button && el_exit_full_tab_button) {
				el_enter_full_tab_button.title = "拉伸到整个选项卡";
			...
			if (el_full_screen_button && __getFullScreenFunction(el_window)) {
				el_full_screen_button.title = "进入全屏模式";
	...
	self.custom = (...args) => __modalDialog(...args);

	var __modalDialog = function(header, text, ok, cancel, parent, options) {
		...
		if (ok || cancel) {
			promise = new Promise(function(resolve) {
				...
				if (cancel) {
					...
					el_cancel_button.innerHTML = (options && options.noText) || "取消";
				...
				if (ok) {
					...
					el_ok_button.innerHTML = (options && options.okText) || "确定";
					tools.el.setOnClick(el_ok_button, () => close(true));
					el_buttons.appendChild(el_ok_button);
				}
				if (ok && cancel) {
					el_ok_button.className = "row50" + (options && options.okCls ? ' ' + options.okCls : '');
